                ...
                VStack(spacing: 12) {
                    ForEach(Array(day.exercises.enumerated()), id: \.element.id) { index, exercise in
                        NavigationLink(destination:
                            ...
                            ExerciseRowCard(exercise: exercise, index: index + 1, lm: lm)
                        }
                        .buttonStyle(PlainButtonStyle())
                    }
                }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                HStack {
                    VStack(alignment: .leading) {
                        Text(exercise.category.rawValue).font(.caption).foregroundColor(.secondary)
    ...
    var body: some View {
        HStack {
            Text("\(index)").font(.caption).bold()
                ...
                .background(Color.green.opacity(0.15)).foregroundColor(.green).clipShape(Circle())
            VStack(alignment: .leading, spacing: 2) {
